...
Write-Host "Building LU NIF Viewer..."
& cmake -S $repositoryRoot -B $buildDirectory
if ($LASTEXITCODE -ne 0) {
    throw "CMake configuration failed with exit code $LASTEXITCODE."
}

& cmake --build $buildDirectory --target lu_nif_viewer --config RelWithDebInfo -- /m:1
if ($LASTEXITCODE -ne 0) {
    throw "Viewer build failed with exit code $LASTEXITCODE."
}

if (-not (Test-Path -LiteralPath $viewerExecutable)) {
    throw "The viewer was not built at: $viewerExecutable"
}
